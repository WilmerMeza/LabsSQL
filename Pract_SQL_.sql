...
-- 5.- Visualizar listado de subcategorías con sus respectivas categorías
         ...
         SELECT ps.NAME AS 'SubCategoria', pc.NAME  AS 'Categoria'
...
-- 6. Visualizar cantidad de productos por cada subcategoría
       SELECT S.Name AS SUBCATEGORIA, COUNT(P.ProductID) AS CANTIDAD  
	   FROM Production.ProductSubcategory AS S
       INNER JOIN Production.Product AS
       P ON P.ProductSubcategoryID = S.ProductSubcategoryID GROUP BY S.Name

-- 7. Ver precio promedio por cada categoría de producto
      SELECT C.Name,AVG(StandardCost) 
      AS Precio_Promedio FROM Production.Product P
...
-- 8. Ver cantidad de productos por categoría
      ...
      SELECT C.Name AS Categoria,COUNT(P.ProductID) 
	  AS CANTIDAD FROM Production.Product P
      INNER JOIN Production.ProductSubcategory S
      on S.ProductSubcategoryID = P.ProductSubcategoryID
      INNER JOIN Production.ProductCategory C
      on C.ProductCategoryID = S.ProductCategoryID
...
-- 10. Visualizar el total de ventas por cada categoría de producto
      ...
      SELECT C.Name, SUM(SA.LineTotal * SA.UnitPrice)
      as TOTAL
...
-- 11. Ver la cantidad total de empleados
      ...
      SELECT COUNT(H.BusinessEntityID) 
      AS TOTAL_EMPLEADOS 
      FROM HumanResources.Employee H;

--12. Ver la cantidad total de empleados de acuerdo a su estado civil.

      SELECT MaritalStatus, COUNT(H.BusinessEntityID) 
      AS ESTADO_CIVIL from HumanResources.Employee H GROUP BY MaritalStatus ;

--13. Ver cantidad de empleados por género

       SELECT Gender, COUNT(H.BusinessEntityID) 
       AS Genero from HumanResources.Employee H GROUP BY Gender ;
...
--15. Ver cantidad de empleados por cada departamento
       ...
       SELECT D.Name AS Departamento, COUNT(E.BusinessEntityID) 
       as Empleados from HumanResources.Employee E 
